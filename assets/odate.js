/* Modified for standalone use from https://github.com/Financial-Times/o-date */
(function(){
...
let interval;
window.ODate = function ODate(rootEl) {
	if (!rootEl) {
	...
	if (!interval) {
		...
		}, 5000); // modified from original 60s to 5s
	}
}
...
ODate.timeAgo = function(date, interval) {
  ...
  if (interval < 10) {
    return 'just now'; // added special condition for < 10 seconds
  } else if (interval < inSeconds.minute) {
